...
import type { PensionRow } from '../types/dashboard';
...
/**
 * 연금 CSV 행 배열을 연금 현황 탭용 PensionRow[]로 변환합니다.
 * 컬럼 구조: 상품명, 투자시점(등), 투자원금, 평가금액, 수익률, 이후 6개 월별 수익률.
 * 상품 행만 남기고(개인연금 합계·날짜·빈 행 제외) 데이터 순서대로 상한까지 노출합니다.
 */
export function pensionToRows(rows: SheetDataRow[]): PensionRow[] {
  ...
  return detailRows.map((row, i) => {
    ...
    const sparklineData = getProductSparklineData(name, 'PENSION', 6);
    const fallbackSparkline =
      sparklineData.length >= 2 ? sparklineData : [0, 0, 0, 0, 0, returnRate];
    const monthlyDeltas =
