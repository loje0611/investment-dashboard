...
export function ElsRedeemModal({ open, onClose, rowIndex, productName, defaultRedeemAmount, onSuccess }: ElsRedeemModalProps) {
  ...
  return (
    <BottomSheet open={open} onClose={handleClose} title="ELS 상환" titleId="els-redeem-title">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-5 py-4">
        <p className="text-sm text-content-secondary">
          <span className="font-medium text-content-primary">{productName}</span>
        </p>
        <div>
          <label htmlFor="els-redeem-date" className="mb-1.5 block text-sm font-medium text-content-secondary">상환일</label>
        ...
        <div>
          <label htmlFor="els-redeem-amount" className="mb-1.5 block text-sm font-medium text-content-secondary">상환금액 (원)</label>
        ...
        <p className="text-xs leading-relaxed text-content-tertiary">
          저장 시 시트에 상태「상환완료」, 상환일·상환금액이 반영되고, 가입일·가입금액 기준으로 투자기간·연수익률·수익이 계산되어 입력됩니다.
        </p>
