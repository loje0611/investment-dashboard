...
export function LoginPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-slate-100 to-slate-200/90">
      <div className="mx-auto flex w-full max-w-[480px] flex-1 flex-col justify-center px-6 py-12">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-8 shadow-[0_8px_40px_rgba(15,23,42,0.08)]">
          ...
          <h1 className="mt-2 text-center text-2xl font-bold text-slate-900">로그인</h1>
          ...
          {!clientId && (
            <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
              ...
              <p className="mt-1 text-xs leading-relaxed text-amber-900/90">
                ...
                <code className="rounded bg-amber-100/80 px-1">.env</code>에{' '}
                <code className="rounded bg-amber-100/80 px-1">VITE_GOOGLE_CLIENT_ID</code>를
          ...
          <button
            ...
            onClick={() => googleLogin()}
            className="mt-8 flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white py-3.5 text-sm font-semibold text-slate-800 shadow-sm transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
