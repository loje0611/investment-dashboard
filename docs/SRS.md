...
### Task 8: 원금 및 현금 자산 직접 갱신 스크립트 (Python) 구축
- **목표**: 프론트엔드 UI의 임시 덮어쓰기(localStorage) 방식의 한계를 극복하기 위해, 사용자가 터미널 환경에서 직관적으로 CSV 원본 데이터(원금 등)를 수정할 수 있는 CLI 도구를 제공합니다.
- **구현 내용**:
  - `scripts/update_principal.py` (또는 기존 스크립트 기능 확장) 작성
  - **대화형 CLI 프롬프트 흐름**: 스크립트 실행 시 다음과 같은 형태로 항목별 변경 여부를 묻습니다.
    ```text
    =========================================
      투자 원금 및 현금 자산 업데이트 도구
    =========================================
    * 변경이 없는 항목은 그대로 Enter(엔터)를 누르세요.

    [1/3] 현금 자산
    - 최근 현금 평가금: 5,432,000 원
    > 새로운 현금 입력: 

    [2/3] ETF/자문사 상품 원금
    - 풍차1 (현재 원금: 4,000,000 원)
    > 새로운 원금 입력: 4500000
    - 풍차2 (현재 원금: 4,000,000 원)
    > 새로운 원금 입력: 

    [3/3] 연금 상품 원금
    - KODEX TDF2050액티브 (현재 원금: 1,500,000 원)
    > 새로운 원금 입력: 

    ✅ CSV 원본 업데이트가 완료되었습니다! (history.csv, portfolio.csv)
    ```
  - 사용자가 빈 값(Enter)을 넘기면 기존 값을 유지하고, 새로운 숫자 값을 입력하면 해당 값을 갱신 적용
  - 최종 완료 시 실제 `portfolio.csv`(개별 상품 원금)와 `history.csv`(전체 현금 자산 이력 등) 파일을 직접 덮어써 영구적으로 기록 (Single Source of Truth 유지)
- **Acceptance Criteria**:
  - [ ] 스크립트 실행 시 터미널에서 위 명세와 유사한 형태의 대화형 프롬프트가 출력되어야 한다.
  - [ ] 값 입력 완료 후 즉시 원본 CSV 파일(history.csv, portfolio.csv)이 변경사항에 맞게 정상적으로 업데이트되어야 한다.
  - [ ] 대시보드를 새로고침했을 때 스크립트로 변경된 데이터가 아무런 추가 조작 없이 즉시 반영되어 화면에 나타나야 한다.
